	...
	schema := map[string]any{
		...
		"properties": map[string]any{
			...
			"tape_name": map[string]any{
				"type":        "string",
				"description": "Required when not in a Feishu-triggered RunAgent. Must be feishu:p2p:<chat_id>. Mutually exclusive with chat_id.",
			},
...
func ExecuteSendText(ctx context.Context, argsJSON string, jobChatID string, jobTriggerMessageID string, jobInThread bool, jobBot ThreadAwareMessageClient, getBotForChat func(string) (SimpleMessageClient, error)) (map[string]any, error) {
	...
	if jobBot != nil && jobChatID != "" {
		...
		if tapeName != "" || chatIDArg != "" {
			return nil, fmt.Errorf("do not set tape_name or chat_id during a Feishu-triggered RunAgent")
		}
	...
	switch {
	case tapeName != "":
		// Extract chat_id from tape_name "feishu:p2p:<chat_id>"
		const prefix = "feishu:p2p:"
		if !strings.HasPrefix(tapeName, prefix) {
			return nil, fmt.Errorf("tape_name must start with %q", prefix)
		}
		chatID = strings.TrimSpace(tapeName[len(prefix):])
		// Strip subagent suffix if present
		if idx := strings.Index(chatID, ":"); idx > 0 {
			chatID = chatID[:idx]
		}
		if chatID == "" {
			return nil, fmt.Errorf("tape_name has empty chat id after prefix")
		}
	case chatIDArg != "":
	...
	if err != nil {
		return nil, fmt.Errorf("feishuSendText: %w (chat_id=%q)", err, chatID)
	}
